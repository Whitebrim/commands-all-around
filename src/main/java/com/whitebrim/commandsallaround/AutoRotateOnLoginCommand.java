...
public class AutoRotateOnLoginCommand extends CommandBase {

    @Override
    public void execute(MinecraftServer server, ICommandSender sender, String[] params) throws CommandException {
        if(params.length > 0){
            ConfigHandler.writeConfig("Auto rotate", "rotate on login", parseBoolean(params[0]));
            sender.sendMessage(new TextComponentTranslation("commands.commandsallaround.success"));
        }else{
            sender.sendMessage(new TextComponentString(String.valueOf(ConfigHandler.getBoolean("Auto rotate", "rotate on login"))));
        }
    }
    ...
    public String getName() {
        return "autoRotateOnLogin";
    }

    @Override
    public String getUsage(ICommandSender sender) {
        return "commands.autorotateonlogin.usage";
    }
}
